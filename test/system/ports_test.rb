...
class PortsTest < ApplicationSystemTestCase
  ...
  test "creating a Port" do
    ...
    fill_in "Comfort time", with: @port.comfort_time
    fill_in "Coordinates", with: @port.coordinates
    fill_in "Cost", with: @port.cost
    check "Electric" if @port.electric
    fill_in "Name", with: @port.name
    fill_in "Rating", with: @port.rating
    fill_in "Tel", with: @port.tel
    check "Wather" if @port.wather
    fill_in "Workload", with: @port.workload
